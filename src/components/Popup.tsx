...
export default function Popup({popupState, onClick}: {popupState:{open:boolean}, onClick:()=>void}) {
    ...
    if (popupState.open) return (
        <>
            ...
            <div className="h-max w-max p-5 bg-white text-slate-600 dark:bg-slate-800 dark:text-slate-100  absolute top-0 left-0 right-0 bottom-0 m-auto z-20 rounded-xl" onClick={()=>{}}>
                {uploadDiv}
                <form method="POST" action="http://localhost:3000/api/upload">
                    <div className={'flex flex-col'}>
                        ...
                        <label htmlFor={'description'} className={'mt-2'}>Opis zdjęcia</label>
